...
nvim --headless -u NONE -i NONE \
  ...
  --cmd "cd $TEST_DIR" \
  "+lua local ok, err = pcall(function() package.loaded['dap'] = { adapters = { python = {} }, run = function(cfg) vim.g.cplug_python_layout = cfg end }; package.loaded['dapui'] = { open = function() end }; require('cplug').setup({ launch = { on_missing = 'always' }, python = { bootstrap_debugpy = false } }); local result, run_err = require('cplug').compile_and_debug(); assert(result, run_err); assert(result.backend == 'python'); assert(vim.fn.filereadable('.vscode/launch.json') == 1); assert(vim.g.cplug_python_layout.program == '\${workspaceFolder}/src/pkg/app.py'); local launch = vim.fn.readfile('.vscode/launch.json'); assert(#launch > 1); assert(not table.concat(launch, '\n'):find('ignored.py', 1, true)) end); if not ok then vim.api.nvim_err_writeln(err); vim.cmd('cquit 1') end" \
  +qall
