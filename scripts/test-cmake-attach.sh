...
nvim --headless -u NONE -i NONE --cmd "set rtp+=$ROOT_DIR" \
  "+cd $TEST_DIR_REAL" \
  "+lua local ok, err = pcall(function() local original_system = vim.system; vim.system = function() error('vim.system should not be called during attach') end; package.loaded['dap'] = { run = function(cfg) vim.g.cplug_cmake_attach = cfg end }; package.loaded['dapui'] = { open = function() end, setup = function() end, close = function() end }; package.loaded['dap-disasm'] = { setup = function() end }; require('cplug').setup({ launch = { on_missing = 'always' } }); local result, attach_err = require('cplug').attach(); vim.system = original_system; assert(result, attach_err); assert(result.backend == 'cmake'); assert(result.dap.low_level == true); assert(result.dap.layout == 'native'); assert(vim.g.cplug_cmake_attach.request == 'attach'); assert(vim.g.cplug_cmake_attach.pid == '\${command:pickProcess}'); assert(vim.fn.filereadable('.vscode/launch.json') == 1); local launch = vim.json.decode(table.concat(vim.fn.readfile('.vscode/launch.json'), '\n')); assert(#launch.configurations == 1); assert(launch.configurations[1].request == 'attach'); assert(launch.configurations[1].pid == '\${command:pickProcess}') end); if not ok then vim.api.nvim_err_writeln(err); vim.cmd('cquit 1') end" \
  +qall
