...
local ns = vim.api.nvim_create_namespace("cplug.launch_config_picker")
...
local function render()
  ...
  if #state.filtered == 0 then
    lines[1] = "  No matching launch configurations"
  else
...
local function close(selected_entry)
...
function M.open(opts)
  ...
  vim.bo[state.prompt_buf].filetype = "cplug_launch_config_picker"
  vim.fn.prompt_setprompt(state.prompt_buf, state.prompt)

  vim.bo[state.list_buf].bufhidden = "wipe"
  vim.bo[state.list_buf].filetype = "cplug_launch_config_picker"
  vim.bo[state.list_buf].modifiable = false

  state.prompt_win = create_window(state.prompt_buf, {
    enter = true,
    title = "Cplug Launch Configurations",
    width = width,
    height = 1,
    row = row,
    col = col,
  ...
  map({ "i", "n" }, "<Up>", function()
    move_selection(-1)
  end, "Previous launch configuration")
  ...
  map({ "i", "n" }, "<CR>", select_current, "Select launch configuration")
  map("i", "<C-w>", "<C-S-w>", "Delete word")

  local group = vim.api.nvim_create_augroup("cplug.launch_config_picker." .. state.prompt_buf, { clear = true })
...
function M.pick(opts)
  ...
  local wait_ok = vim.wait(opts.timeout_ms or 24 * 60 * 60 * 1000, function()
    return selected ~= nil or cancelled
  end, 10)

  if not wait_ok then
    M.close()
    return nil, "Launch configuration selection timed out"
  end

  if cancelled then
    return nil, "Launch configuration selection cancelled"
  end
