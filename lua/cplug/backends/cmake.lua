...
local function render_gitignore(project)
  local build_dir = relative_path(project.root, project.build_dir)

  return {
    "# Generated by cplug.nvim during C/C++ scaffolding",
    ("%s/"):format(build_dir),
    "*.swp",
    "*~",
    ".DS_Store",
...
function M.detect(ctx)
  ...
  local gitignore = vim.fs.joinpath(ctx.cwd, ".gitignore")
  local git_dir = vim.fs.joinpath(ctx.cwd, ".git")
...
local function bootstrap_git(project)
  if not project.config.bootstrap_git then
    return true
  end

  if not file_exists(project.gitignore) then
    local gitignore_ok = vim.fn.writefile(render_gitignore(project), project.gitignore)

    if gitignore_ok ~= 0 then
      return nil, ("Failed to write `%s`"):format(project.gitignore)
    end
  end

  if path_exists(project.git_dir) then
    return true
  end

  if vim.fn.executable("git") ~= 1 then
    return nil, "Git bootstrap requested, but `git` is not available"
  end

  local _, git_err = run_command({ "git", "init", "--quiet" }, project.root)

  if git_err then
    return nil, ("Git init failed: %s"):format(git_err)
  end
...
function M.scaffold(ctx, project)
  ...
  local git_ok, git_err = bootstrap_git(project)
